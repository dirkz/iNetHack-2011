		...
		while (object) {
			if (Is_container(object)) {
				if (Is_box(object)) { // not a bag or medkit
					...
					[self addCommand:[NhCommand commandWithTitle:"Untrap Container" keys:cmdUntrapDown] toCommands:commands key:kFloor];
					if (object->olocked) {
						if (inv & fWieldedWeapon) {
							char forceDown[] = {M('f'), '>', 'y', 0};
							[self addCommand:[NhCommand commandWithTitle:"Force Container" keys:forceDown] toCommands:commands key:kFloor];
						}
						if (inv & fAppliable) {
							[self addCommand:[NhCommand commandWithTitle:"Apply" key:'a'] toCommands:commands key:kFloor];
						}
					} else {
...
#ifdef SLASHEM
				[self addCommand:[NhCommand commandWithTitle:"Eat what's here" keys:"e,"] toCommands:commands key:kFloor];
#else
				[self addCommand:[NhCommand commandWithTitle:"Eat" keys:"e"] toCommands:commands key:kFloor];
#endif
...
#ifdef SLASHEM
						[self addCommand:[NhCommand commandWithObject:tinningKit title:"Tin what's here" keys:"a" direction:","] toCommands:commands key:kFloor]; 
#else
						[self addCommand:[NhCommand commandWithObject:tinningKit title:"Tin what's here" keys:"a"] toCommands:commands key:kFloor]; 
#endif
					}
				}
			}
